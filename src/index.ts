...
import { UpdataCoursesModel } from "./models/UpdataCoursesModel";
import { QueryCoursesModel } from "./models/QueryCoursesModel";
import { CourseViewModel } from "./models/CourseVeiwModel";
import {URIParamsCourseModel} from './models/URIParamsCourseIdModel'
...
app.get(
  "/courses/:id",
  (req: RequestWithParams<URIParamsCourseModel>, res: Response<CourseViewModel>) => {
...
app.post(
  ...
  (req: RequestWithBody<CreateCourseModel>, res: Response<CourseViewModel>) => {
    ...
    const createCourses: CourseType = {
      id: Number(new Date()),
...
app.delete(
  "/courses/:id",
  function (req: RequestWithParams<{ id: string }>, res) {
...
app.put(
  "/courses/:id",
  function (
    req: RequestWithParamsAndBody<URIParamsCourseModel, UpdataCoursesModel>,
    res
  ) {
    if (!req.body.title) {
      res.sendStatus(404);
